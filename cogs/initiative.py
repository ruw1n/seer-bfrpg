...
class Initiative(commands.Cog):
    ...
    async def monster_card(self, ctx, who: str | None = None):
        """DM yourself the monster attack card (attacks/turn, attacks, spells, specials).

        Usage:
          !mc <monster>      (e.g., !mc sk1)
          !mc               (uses the current turn, if it's a monster)

        Permission: GM (battle DM / Manage Server) or the monster's controller (owner / summoner).
        """
        cfg = _load_battles()
        chan_id = _section_id(ctx.channel)
        if not cfg.has_section(chan_id):
        ...
        if not who:
            who = (cfg.get(chan_id, "turn", fallback="") or "").strip()
            if not who:
                await ctx.send("❌ Usage: `!mc <monster>`")
                return

        resolved, amb = _find_ci_or_partial_name(names, who)
        if amb:
            await ctx.send("❌ Ambiguous. Did you mean: " + ", ".join(f"`{n}`" for n in amb[:10]))
            return
        if not resolved:
            await ctx.send(f"❌ Combatant '{who}' not found here. Use `!list`.")
            return

        if not _is_monster(resolved):
            await ctx.send(f"❌ **{resolved}** is not a monster. (This command is for monster turns/cards.)")
            return

        def _extract_owner_id(cfg2):
            oid = (get_compat(cfg2, "info", "owner_id", fallback="") or "").strip()
            if oid:
                return oid
            legacy = (get_compat(cfg2, "info", "owner", fallback="") or "")
            import re
            m = re.search(r"(\d{15,25})", legacy)
            return m.group(1) if m else ""

        controller_id = ""
        # If it's a summoned/minion creature, use the summoner's owner.
        try:
            slot = _choose_slot_for_effects(cfg, chan_id, resolved)
            summoner_name = (cfg.get(chan_id, f"{slot}.minion_by", fallback="") or "").strip()
            if summoner_name:
                _disp, summ_path = _resolve_char_ci_local(summoner_name)
                if summ_path and os.path.exists(summ_path):
                    pcfg = read_cfg(summ_path)
                    controller_id = _extract_owner_id(pcfg)
        except Exception:
            pass

        # Else, use the monster record's owner.
        if not controller_id:
            try:
                coe, _base_ini, _mtype = _monster_source_files(resolved)
                if coe and os.path.exists(coe):
                    icfg = read_cfg(coe)
                    controller_id = _extract_owner_id(icfg)
            except Exception:
                pass

        is_gm = _is_dm_for_channel(ctx, cfg, chan_id)
        if not is_gm:
            if controller_id:
                if str(ctx.author.id) != str(controller_id):
                    await ctx.send("❌ Only the GM or that monster’s controller can use `!mc`.")
                    return
            else:
                await ctx.send("❌ Only the GM can use `!mc` for unowned monsters.")
                return
        ...
        title = f"Monster Turn: {resolved}"
        embed = nextcord.Embed(title=title, color=random.randint(0, 0xFFFFFF))
        if prof.get("attacks_per_turn") is not None:
            embed.add_field(name="Attacks per Turn", value=str(prof["attacks_per_turn"]), inline=True)
        if prof.get("ac") is not None:
            embed.add_field(name="AC", value=str(prof["ac"]), inline=True)
        if prof.get("move") is not None:
            embed.add_field(name="Move", value=str(prof["move"]), inline=True)
    ...
    async def battle_mode(self, ctx, state: str | None = None):
        ...
        # UX: don't ping when entering combat (people assume ping = their turn),
        # but DO ping when returning to exploration (so folks notice exploration resumed).
        party_pings = ""
        if gi_on and changed_mode and (not new):  # only when switching to exploration
