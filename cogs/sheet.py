...
import re
import glob
from nextcord.ext import commandsimport configparser
import os
import re
import random
import nextcord
...
def setup(bot):
    bot.add_cog(SheetCog(bot))import configparser
import os
import re
import random
import nextcord
...
class SheetCog(commands.Cog):
    ...
    async def charcreate(self, ctx, name: str, race: str, char_class: str, sex: str = None, *extras):
        ...
        item_sec = {}
        storage_names = sorted({self._canon(k).strip() for k in [*storage_items, *(n for n,_ in STARTER_KIT)] if k})
        item_sec["storage"] = " ".join(storage_names)
        for lower_key, cnt in inv_counts.items():
            item_sec[lower_key] = str(cnt)
        config["item"] = item_sec
